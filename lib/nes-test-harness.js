...
// DEBUG: Check which jsnes we loaded
if (process.env.DEBUG) {
    const ppuPath = require.resolve('jsnes/src/ppu.js');
    const ppuSource = fs.readFileSync(ppuPath, 'utf8');
    const hasFix = ppuSource.includes('All palette entry 0 addresses');
    console.error('[HARNESS-INIT] jsnes ppu.js:', ppuPath);
    console.error('[HARNESS-INIT] Has palette fix:', hasFix);
}
...
const commands = {
    ...
    frame: (args) => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            frameCount++;
            if (verbosity >= 1) {
                const ppuStatus = nes.cpu.mem[0x2002];
                console.error(`[HARNESS-DEBUG] Frame ${frameCount}: PPU Status = 0x${ppuStatus.toString(16)}`);
            }
        }
    ...
    getState: () => {
        ...
        // Debug palette extraction
        const paletteSlice = nes.ppu.vramMem.slice(0x3F00, 0x3F20);
        debug(`palette slice length: ${paletteSlice.length}, first few: [${paletteSlice.slice(0, 4).join(',')}]`);

        // Debug: Log PPU status flags for sprite 0 hit detection
        if (verbosity >= 2) {
            console.error(
                '[HARNESS-PPU-STATUS]',
                `vBlank: ${nes.ppu.f_vBlank}, sprite0Hit: ${nes.ppu.f_sprite0Hit}, spriteOverflow: ${nes.ppu.f_spriteOverflow}`
            );
        }
        ...
        const state = {
            ...
            palette: (function() {
                const pal = Array.from(nes.ppu.vramMem.slice(0x3F00, 0x3F20));
                if (verbosity >= 2) {
                    console.error('[HARNESS-DEBUG] Palette 0x3F00=' + pal[0x00] + ', 0x3F04=' + pal[0x04] + ', 0x3F10=' + pal[0x10]);
                }
                return pal;
            })(),
            nametable: (function() {
                // Nametables: $2000-$2FFF (4KB logical space, mirrored to 2KB physical)
                // For simplicity, expose first nametable (960 bytes) + first attribute table (64 bytes)
                // Layout: $2000-$23BF (960 tiles) + $23C0-$23FF (64 attribute bytes) = 1024 bytes
                const nt = Array.from(nes.ppu.vramMem.slice(0x2000, 0x2400));
                if (verbosity >= 2) {
                    console.error('[HARNESS-DEBUG] Nametable 0x2000=' + nt[0x00] + ', 0x2001=' + nt[0x01]);
                }
                return nt;
            })(),
            controllers: {
